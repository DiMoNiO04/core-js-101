...
function logger(func, logFunc) {
  return (...args) => {
    const strArgs = args.map((i) => JSON.stringify(i)).join(',');
    const strForLog = `${func.name}(${strArgs})`;
    logFunc(`${strForLog} starts`);
    const res = func(...args);
    logFunc(`${strForLog} ends`);
    return res;
  };
}
