...
function isInsideCircle(circle, point) {
  const hypotenuse = Math.sqrt((Math.abs(circle.center.x - point.x)) ** 2
...
function isCreditCardNumber(ccn) {
  const numbers = ccn.toString().split('').map((number) => +number);
  let sum = 0;
  let even = false;
...
function getDigitalRoot(num) {
  const result = num.toString().split('').reduce((sum, item) => sum + Number(item), 0);
...
function getCommonDirectoryPath(pathes) {
  if (pathes.every((path) => path[0] === '/')) {
    const itemsArr = pathes.map((path) => path.split('/'));
    const str = itemsArr[0].filter((elem, index) => itemsArr.every((arr) => arr[index] === elem));
...
function evaluateTicTacToePosition(position) {
  ...
  if (position[2][2]) {
    if (
      (position[2][2] === position[2][0] && position[2][2] === position[2][1])
